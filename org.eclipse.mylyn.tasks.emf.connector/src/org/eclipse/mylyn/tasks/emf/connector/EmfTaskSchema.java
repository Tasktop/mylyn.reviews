...
public abstract class EmfTaskSchema extends AbstractTaskSchema {
	...
	private final Field taskKey = null;
	...
	public void initialize() {
		...
		if (!keyFieldDefined) {
			for (EClass taskClass : getSchemaEClasses()) {
				EAttribute idAttribute = taskClass.getEIDAttribute();
				if (idAttribute != null) {
					FieldFeature keyFeature = new FieldFeature(parent.TASK_KEY, idAttribute, true);
					createField(keyFeature);
					break;
				}
			}
		}
