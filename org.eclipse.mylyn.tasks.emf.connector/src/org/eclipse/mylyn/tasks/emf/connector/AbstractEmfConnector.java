...
import org.eclipse.mylyn.tasks.emf.connector.query.EmfQueryEngine;
import org.eclipse.mylyn.tasks.emf.connector.query.QueryClause;
import org.eclipse.mylyn.tasks.emf.connector.query.QueryException;
import org.eclipse.mylyn.tasks.emf.connector.query.SimpleQueryEngine;
...
public abstract class AbstractEmfConnector extends AbstractRepositoryConnector {
	...
	/**
	 * Replace Hyphen with Unicode en-dash.
	 */
	public static final String HANDLE_ENCODE_VALUE = "\u2013"; //$NON-NLS-1$
	...
	public class EmfTaskSchemaDelegator extends EmfTaskSchema {
		...
		protected EClass[] getSchemaEClasses() {
			return AbstractEmfConnector.this.getSchemaEClasses();
		}

		@Override
		protected FieldFeature[] getSchemaPairs() {
			return AbstractEmfConnector.this.getSchemaPairs();
		}
	...
	public boolean initializeTaskData(TaskRepository repository, TaskData data, ITaskMapping initializationData,
		...
		data.getRoot().createAttribute(TaskAttribute.TASK_KEY).setValue(getTaskKeyFromTaskId(taskId));
		return true;
	}

	public EObject getTaskObject(TaskRepository repository, EObject container, String taskId, IProgressMonitor monitor) {
		for (Object object : ((List<?>) container.eGet(getContainmentReference()))) {
			if (object instanceof EObject) {
				EObject eObject = (EObject) object;
				String stringValueForKey = getEmfMapper(repository).getEmfString(eObject, TaskAttribute.TASK_KEY);
	...
	public RepositoryResponse postTaskData(TaskRepository repository, TaskData taskData,
		...
		client.open();
		EObject emfTask = getTaskObjectChecked(repository, taskData.getTaskId(), monitor);
...
//			TaskAttribute taskDate = taskData.getRoot().getMappedAttribute(TaskAttribute.DATE_MODIFICATION);
//			taskDate.clearValues();
//			getEmfMapper(repository).copyEmfToTask(emfTask, dateModificationFeature, taskData);
	...
	public TaskData getTaskData(TaskRepository repository, String taskId, IProgressMonitor monitor)
		...
		String id = getEmfMapper(repository).getEmfString(emfObject, TaskAttribute.TASK_KEY);
	...
	private EObject getTaskObjectChecked(TaskRepository repository, String taskId, IProgressMonitor monitor)
			throws CoreException {
		String decodeTaskId = getTaskIdFromTaskKey(taskId);
		EObject emfObject = getTaskObject(repository, getClient(repository).getRootContainer(), decodeTaskId, monitor);
		if (emfObject == null) {
	 ...
	 * @return
	 */
	public String getTaskKeyFromTaskId(String taskId) {
		return StringUtils.replace(taskId, RepositoryTaskHandleUtil.HANDLE_DELIM, HANDLE_ENCODE_VALUE);
	}
	 ...
	 * @return
	 */
	public String getTaskIdFromTaskKey(String taskId) {
		return StringUtils.replace(taskId, HANDLE_ENCODE_VALUE, RepositoryTaskHandleUtil.HANDLE_DELIM);
	}
...
//			taskId = StringUtils.replace(taskId, ITEM_DELIM + HANDLE_ENCODE_VALUE,
//					RepositoryTaskHandleUtil.HANDLE_DELIM);
//			taskId = StringUtils.replace(taskId, ITEM_DELIM + ITEM_DELIM, ITEM_DELIM);
			return taskId;
		}
		return null;
	}
	...
	public String getTaskUrl(String repositoryUrl, String taskId) {
		String encodedId = taskId; //StringUtils.replace(taskId, ITEM_DELIM, ITEM_DELIM + ITEM_DELIM);
		//encodedId = StringUtils.replace(taskId, RepositoryTaskHandleUtil.HANDLE_DELIM, ITEM_DELIM + HANDLE_ENCODE_VALUE);
		try {
...
//			new SELECT(new WHERE(new EObjectAttributeValueCondition(attribute, valueCondition)), where)
			...
			try {
				...
				for (EObject taskObject : results) {
					String id = getTaskKeyFromTaskId(getEmfMapper(client.getRepository()).getEmfString(taskObject,
...
//			task.setTaskKey(mapper.getTaskKey());
	...
	public abstract EAttribute getTaskIdAttribute();

	public abstract EAttribute[] getSearchAttributes();

	public abstract Bundle getConnectorBundle();

	public abstract EClass[] getSchemaEClasses();

	public abstract FieldFeature[] getSchemaPairs();
}
