 ...
 *******************************************************************************/
package org.eclipse.mylyn.tasks.emf.connector.ui.wizards;
...
import org.eclipse.mylyn.tasks.emf.connector.EmfAttributeMapper;
...
import org.eclipse.mylyn.tasks.emf.connector.query.QueryOperation;
import org.eclipse.mylyn.tasks.internal.emf.connector.ui.EmfUiPlugin;
 ...
 */
public abstract class EmfBaseQueryPage extends AbstractRepositoryQueryPage2 {
		...
		void applyTo(IRepositoryQuery query) {
			if (!textControl.getText().equals("")) { //$NON-NLS-1$
				...
				query.setAttribute(getConnector().getSchema().getKey(attribute) + EmfQueryEngine.QUERY_DELIMITER
						+ EmfQueryEngine.QUERY_OPERATION, selectedOperation.getId());
				query.setAttribute(getConnector().getSchema().getKey(attribute) + EmfQueryEngine.QUERY_DELIMITER
						+ EmfQueryEngine.QUERY_VALUE, textControl.getText());
			}
		}
	...
	class EmfDateControl extends EmfControl {
		...
		void applyTo(IRepositoryQuery query) {
			...
			if (selectedOperation == QueryOperation.IN_RANGE) {
				query.setAttribute(getConnector().getSchema().getKey(attribute) + EmfQueryEngine.QUERY_DELIMITER
						+ EmfQueryEngine.QUERY_VALUE, date1 + EmfQueryEngine.LIST_ITEM_SEPERATOR + date2);
			} else if (selectedOperation == QueryOperation.BEFORE) {
				query.setAttribute(getConnector().getSchema().getKey(attribute) + EmfQueryEngine.QUERY_DELIMITER
						+ EmfQueryEngine.QUERY_VALUE, date2);
			} else if (selectedOperation == QueryOperation.AFTER) {
				query.setAttribute(getConnector().getSchema().getKey(attribute) + EmfQueryEngine.QUERY_DELIMITER
						+ EmfQueryEngine.QUERY_VALUE, date1);
			}
		}
	...
	public EmfBaseQueryPage(TaskRepository repository, String pageName, IRepositoryQuery query) {
